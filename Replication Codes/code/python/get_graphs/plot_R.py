...
mask = (df['Country/Region'] == "Indonesia") & (df['days_infectious'] == days_infectious)
df_temp = df.loc[mask, ].copy()
start_epidemic = df_temp['Date'].min()
mask = (df_temp['R'] < 1)
date_below_one = df_temp.loc[mask, 'Date'].min()
diff = (date_below_one - start_epidemic).days
days_to_below_one.append({'Country/Region': "Indonesia",
            'days_to_below_one': diff})
...
for country in df['Country/Region'].unique():
    print(country)
    if (country == "Indonesia") or (country == "World"):
        mask = (df['Country/Region'] == country) & (df['days_infectious'] == days_infectious)
        df_temp = df.loc[mask, ].copy()
        ...
        plt.title("Estimasi Nilai Reproduction Rate COVID-19 di Indonesia")
