...
output_folder = './output/estimate_R_STAN'
input_folder = './input/estimate_R_STAN'

min_T = 20
#days_infectious_list = [5, 6, 7, 8, 9, 10] # Values of (1 / gamma) used in constructing
                                           # time series of infected individuals
days_infectious_list = [7] # Values of (1 / gamma) used in constructing
...
for days_infectious in days_infectious_list:
  ...
  negara = ["World", "Yemen", "Zambia", "Zimbabwe"]

  # Loop over countries
  for country in df['Country/Region'].unique():
    print(country)
    if(country in negara):
